...
public class LobbyScoreboardManager {
    ...
    private Rank getPlayerRank(Player player) {
        ...
        if (plugin.getMongoConnection() != null && plugin.getMongoConnection().isConnected()) {
            try {
                var doc = plugin.getMongoConnection().getDatabase()
                        .getCollection("accounts")
                        .find(com.mongodb.client.model.Filters.eq("username_lower", userLower))
                        .first();
